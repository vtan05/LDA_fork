...
def calc_and_save_feats(root):
    ...
    for bvh in bvh_files:
        ...
        print(joint3d.shape)
        np.save(os.path.join(root, 'kinetic_features', bvh), extract_kinetic_features(joint3d.reshape(-1, 22, 3)))
        np.save(os.path.join(root, 'manual_features_new', bvh), extract_manual_features(joint3d.reshape(-1, 22, 3)))


def process_motion(motion_path):

    fps = 30
    joints = ['Spine1','Spine2','Spine3','Neck','Head',
    ...
    joint_pos = np.reshape(piped_data, [-1, 22, 3]) * 0.01
    print("joint_pos:", joint_pos.shape)  # (T, 22, 3)
    roott = joint_pos[:, 0, :]
...
if __name__ == '__main__':
    ...
    gt_root = '/host_data/van/LDA/data/edge_aistpp/bvh'
    pred_root = '/host_data/van/LDA/results/aist_8s'
