...
checkpoint=/host_data/van/LDA/model/aist/lightning_logs/version_0/checkpoints/epoch=8-step=25749.ckpt
dest_dir=/host_data/van/LDA/results/aist_8s

if [ ! -d "${dest_dir}" ]; then
    mkdir -p "${dest_dir}"
fi

data_dir=/host_data/van/LDA/data/edge_aistpp/feat
wav_dir=/host_data/van/LDA/data/edge_aistpp/wavs
basenames=$(cat "${data_dir}/crossmodal_test.txt")
...
do
	...
	do
		...
		echo seed=$seed
		wavfile_stripped=${wavfile#aist_}
		python utils/cut_wav.py ${wav_dir}/${wavfile_stripped}.wav $(((start+trim)/fps)) $(((start+length-trim)/fps)) ${postfix} ${dest_dir}
		if [ "$render_video" == "true" ]; then
			ffmpeg -y -i ${dest_dir}/${output_file}.mp4 -i ${dest_dir}/${wavfile_stripped}_${postfix}.wav ${dest_dir}/${output_file}_audio.mp4
